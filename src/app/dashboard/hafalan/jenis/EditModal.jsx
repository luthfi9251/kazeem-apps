...
import { addJenisHafalan, editJenisHafalan } from "@/actions/hafalan";
...
export default function EditModal({ open, onOpenChange, data }) {
    const formHafalan = useForm({
        ...
        values: {
            jenis_hafalan: data?.jenis_hafalan ?? "",
        },
    });

    const onSubmitHandler = (formData) => {
        toast.promise(
            () => editJenisHafalan(data?.id, formData),
            {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Ubah Jenis Hafalan</DialogTitle>
                </DialogHeader>
                <Form {...formHafalan}>
                    <form
