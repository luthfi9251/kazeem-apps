...
const getIsDemoApp = async () => {
    return process.env.IS_DEMO_WEBSITE === "true" ? true : false;
};

export default async function Home() {
    const isDemo = await getIsDemoApp();
    return (
        <main className="flex justify-center items-center min-h-screen max-w-screen bg-kazeem-primary">
            <Card className="w-4/5 md:w-1/2 p-1 md:p-4">
                    ...
                    {isDemo && (
                        <p className=" font-normal text-xs text-center flex flex-col bg-yellow-300 py-2 rounded">
                            <span className="flex gap-2 justify-center">
                                ...
                                ditambahkan akan terhapus secara otomatis.
                                Silahkan Masuk dengan akun berikut.
                            </span>
